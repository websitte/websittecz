...
def createBuildInvoice(request, slug):
    ...
    if request.method == 'POST':
        ...
        if prod_form.is_valid():
            ...
            messages.success(request, "Faktura byla úspěšně aktualizována")
            return redirect('create-build-invoice', slug=slug)
...
def emailDocumentInvoice(request, slug):
    ...
    messages.success(request, "Email jsem odeslal klientovi v pořádku")
    return redirect('create-build-invoice', slug=slug)
...
#    except:
#        messages.error(request, 'Něco se pokazilo')
#        return redirect('invoices')
#
#    return redirect('invoices')


class DeleteInvoiceConfirmView(TemplateView):
    template_name = 'invoice/invoice_confirm_delete.html'

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        slug = kwargs.get('slug')
        context['invoice'] = get_object_or_404(Invoice, slug=slug)
        return context
...
def delete_invoice(request, slug):
    if request.method == 'POST':
        ...
        return render(request, 'invoice/invoice_confirm_delete.html')

def companySettings(request):
    company = Settings.objects.get(clientName='Websitte')
